...
saveRDS(comparisonDA,file = paste0("../data/16Ssubsets_replicability_DA_subgingival_supragingival",task_id,"_mixMC.RDS"))

comparison <- subsets_consistency_replicability_16S$gingiva_mucosa[[task_id]]
comparisonDA <- lapply(comparison,mixMCmodel)
saveRDS(comparisonDA,file = paste0("../data/16Ssubsets_replicability_DA_gingiva_mucosa",task_id,"_mixMC.RDS"))

comparison <- subsets_consistency_replicability_16S$tonguedorsum_stool[[task_id]]
comparisonDA <- lapply(comparison,mixMCmodel)
saveRDS(comparisonDA,file = paste0("../data/16Ssubsets_replicability_DA_tonguedorsum_stool",task_id,"_mixMC.RDS"))
