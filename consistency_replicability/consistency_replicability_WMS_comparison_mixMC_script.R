...
saveRDS(comparisonDA,file = paste0("../data/WMSsubsets_replicability_DA_CRC_control",task_id,"_mixMC.RDS"))

comparison <- subsets_consistency_replicability_WMS$schizophrenia_control[[task_id]]
comparisonDA <- lapply(comparison,mixMCmodel)
saveRDS(comparisonDA,file = paste0("../data/WMSsubsets_replicability_DA_schizophrenia_control",task_id,"_mixMC.RDS"))

comparison <- subsets_consistency_replicability_WMS$tonguedorsum_stool[[task_id]]
comparisonDA <- lapply(comparison,mixMCmodel)
saveRDS(comparisonDA,file = paste0("../data/WMSsubsets_replicability_DA_tonguedorsum_stool",task_id,"_mixMC.RDS"))
