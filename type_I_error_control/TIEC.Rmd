...
if(!file.exists("../data/Stool_16S_WMS_mock.RDS")){
  ps_list <- lapply(Stool_16S_WMS,FUN = function(ps){
    mock_list <- list()
    set.seed(123)
    for(i in 1:1000){
      ...
      mock_list <- c(mock_list,ps_obj)
    }
    names(mock_list) <- paste0("Comparison",1:1000)
    mock_list
  })
  saveRDS(object = ps_list, file = "../data/Stool_16S_WMS_mock.RDS")
} else ps_list <- readRDS(file = "../data/Stool_16S_WMS_mock.RDS")
```

Use "sbatch mock_comparison.bash" with "mock_16S_comparison_script.R" and "mock_WMS_comparison_script.R" to evaluate mock datasets. An output is generated for each evaluation.
...
if(!file.exists("../data/Stool_16S_mockDA.RDS")){
  Stool_16S_mockDA <- lapply(ps_list$Stool_16S, oneSimRunGSOwn, epsilon = 1e10)
...
if(!file.exists("../data/Stool_WMS_mockDA.RDS")){
  Stool_WMS_mockDA <- lapply(ps_list$Stool_WMS, oneSimRunGSOwn, epsilon = 1e10)
...
produce_pval_df <- function(ps_fitted_list,pval_type){
  ...
  pval_df <- ldply(pval_df_list,function(x) x, .id = "Comparison")
  ...
  pval_df_qq <- ddply(.data = pval_df,.variables = ~ Comparison + Method,.fun = function(x){
    x <- x[order(x$pval),]
...
plot_FDRcheck <- function(dataset,tech){
  pval_df_FDR_mean <- produce_pval_df(ps_fitted_list = dataset,pval_type = "raw")$pval_df_FDR_mean
...
plot_qq <- function(dataset,tech,zoom = c(0,1)){
  pval_df_qq_summary <- produce_pval_df(ps_fitted_list = dataset,pval_type = "raw")$pval_df_qq_summary
...
plot_KS <- function(dataset,tech){
  pval_df_KS_summary <- produce_pval_df(ps_fitted_list = dataset,pval_type = "raw")$pval_df_KS_summary
...
legend_method <- function(nrow){
  pval_df_KS_summary <- produce_pval_df(ps_fitted_list = Stool_16S_mockDA,pval_type = "raw")$pval_df_KS_summary
...
e <- plot_FDRcheck(dataset = Stool_16S_mockDA,tech = "16S")
f <- plot_FDRcheck(dataset = Stool_WMS_mockDA,tech = "WMS")

a1 <- plot_qq(dataset = Stool_16S_mockDA,tech = "16S",zoom = c(0,1)) + 
  ggtitle(label = paste("QQ-plot 16S stool samples"),subtitle = paste("From 0 to 1"))
a2 <- plot_qq(dataset = Stool_16S_mockDA,tech = "16S",zoom = c(0,0.1)) + 
  ggtitle(label = element_blank(),subtitle = paste("From 0 to 0.1"))
a <- plot_grid(plotlist = list(a1,a2),nrow = 2,align = "hv")

c1 <- plot_qq(dataset = Stool_WMS_mockDA,tech = "WMS",zoom = c(0,1)) +
  ggtitle(label = paste("QQ-plot WMS stool samples"),subtitle = paste("From 0 to 1"))
c2 <- plot_qq(dataset = Stool_WMS_mockDA,tech = "WMS",zoom = c(0,0.1)) + 
  ggtitle(label = element_blank(),subtitle = paste("From 0 to 0.1"))
c <- plot_grid(plotlist = list(c1,c2),nrow = 2,align = "vh")

b <- plot_KS(dataset = Stool_16S_mockDA,tech = "16S")
d <- plot_KS(dataset = Stool_WMS_mockDA,tech = "WMS")
