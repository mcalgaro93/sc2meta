...
if(!file.exists("../data/supragingival_subgingival_DA.RData")){
  ...
  songbird <- read.table(file = "./songbird/16S/differentials.tsv",header = TRUE)
    eval_methods$songbird$statInfo <- data.frame(songbird,row.names = songbird$featureid)
  
  # songbird_df <- songbird[order(songbird$grp.T.grp2.),]
  # songbird_df[,"Methabolism"] <- methabolism_df$Meth[match(songbird_df$featureid,rownames(methabolism_df))]
  # songbird_df$abs_lfc <- -abs(songbird_df$grp.T.grp2.)
  # 
  # ggplot(songbird_df,mapping = aes(x = 1:nrow(songbird_df),y = grp.T.grp2.)) +
  #   geom_col() +
  #   theme_minimal() +
  #   geom_hline(mapping = aes(yintercept = -2.962579),lty = 2, color = "red") +
  #   geom_hline(mapping = aes(yintercept = +2.962579),lty = 2, color = "red")
...
DA features (adjusted p-value < 0.1) are extracted from the methods' list. 5%  top ranked differentials are extracted from songbird.
...
## Adjusted p-value < 0.1 and percent = 5% for songbird
...
plot_grid(plotlist = plot_list_omogeneity,nrow = 3, ncol = 6)
```
...
fig_lor_16S <- ggplot(data = odds_ratio_df %>% filter(Methabolism %in% c("Aerobic","Anaerobic")) ,mapping = aes(x = Method, y = log(Odds.ratio), color = Methabolism)) + 
  ...
  #facet_zoom(ylim = c(-4,4),split = TRUE) +
  geom_hline(mapping = aes(yintercept = 0), color = "black", lty = 2) +
  ggtitle(label = "Log Odds Ratio, Supragingival vs Subgingival Plaque, 16S", subtitle = "Aerobic and Anaerobic taxa focus")
fig_lor_16S
```
...
mf_aerobic <- mutual_findings(DA_methods_df,min_num = 3,DA_feat = c("DOWN in Supragingival","UP in Supragingival"),Meth = "Aerobic",methods = representative_methods)
...
mf_aerobic_supplementary <- mutual_findings(DA_methods_df,min_num = 2,DA_feat = c("DOWN in Supragingival","UP in Supragingival"),Meth = "Aerobic",methods = representative_methods)
```

### Anaerobic

```{r, fig.width=10, fig.height=13}
mf_anaerobic <- mutual_findings(DA_methods_df,min_num = 8,DA_feat = c("DOWN in Supragingival","UP in Supragingival"),Meth = "Anaerobic", methods = representative_methods)
mf_anaerobic

mf_anaerobic_supplementary <- mutual_findings(DA_methods_df,min_num = 5,DA_feat = c("DOWN in Supragingival","UP in Supragingival"),Meth = "Anaerobic", methods = representative_methods)
mf_anaerobic_supplementary
```

```{r, fig.height=12, fig.width=15}
svg(filename = "../../fig_supp_aerobic_anaerobic.svg",width = 15,height = 12)
plot_grid(mf_aerobic_supplementary,mf_anaerobic_supplementary,nrow = 1, labels = "auto")
dev.off()
```
...
mf_fanaerobic <- mutual_findings(DA_methods_df,min_num = 5,DA_feat = c("DOWN in Supragingival","UP in Supragingival"),Meth = "F Anaerobic",methods = representative_methods)
mf_fanaerobic
```
...
# svg(filename = "../../fig6.svg",width = 13,height = 16)
# fig
# dev.off()
```

```{r}
enrichment_summary <- TP_FP_AdjPVal_df[,1:2]
# enrichment_summary$s <- (1-enrichment_summary$s/max(enrichment_summary$s))*17 + 1 

enrichment_summary$s <- rank((1-enrichment_summary$s/max(enrichment_summary$s))*17 + 1)

colnames(enrichment_summary) <- c("method","value")
saveRDS(enrichment_summary, file = "../data/summary/enrichment_summary.RDS")
```
